...
class LikeHComments extends StatefulWidget {
  final String hid;
  final int index;
  final Comment comment;
  final String id;
  const LikeHComments({super.key,
    required this.index,
    required this.hid,
  ...
  @override
  State<LikeHComments> createState() => _LikeHCommentsState();
}

class _LikeHCommentsState extends State<LikeHComments> {
  ...
  Future<void> _toggleLike(bool like) async {
    try {
      if (like) {
        ...
        await hotelService.unlikeComment(widget.hid, widget.index);
      }
