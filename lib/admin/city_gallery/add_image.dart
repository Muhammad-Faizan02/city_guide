...
class AddCityImage extends StatefulWidget {
  final String cId;
  const AddCityImage({super.key, required this.cId});

  @override
  State<AddCityImage> createState() => _AddCityImageState();
}

class _AddCityImageState extends State<AddCityImage> {
  String? _imageUrl;
  File? _pickedImage;
  bool loading = false;
  ...
  Future<void> uploadImgToFirebase(File imageFile) async {
    try {
      ...
      if (taskSnapshot.state == TaskState.success) {
        ...
        setState(() {
          _imageUrl = downloadURL; // Update the imageUrl state variable
          loading = false;
        });
      } else {
            ...
            const Center
              ...
              valueColor: AlwaysStoppedAnimation<Color>(Colors.black),
                ...
                onPressed: ()async{
                 ...
                 await cityService.addImage(widget.cId, _imageUrl!);
               if(mounted){
