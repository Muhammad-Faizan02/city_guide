...
class EventService{
  ...
  EventService({this.eId});


  final CollectionReference eventInformation = FirebaseFirestore.instance.collection('events');


  Stream<List<Event>> get events{
    return eventInformation.snapshots().map(_eventListFromSnapshot);
  }

  Stream<Event> get eventData{
    return eventInformation.doc(eId).snapshots().map(_eventDataFromSnapshot);
  }
  Future createEvent(Event event) async {
    final docEvent = eventInformation.doc();
     event.eId = docEvent.id;
    final json = event.toJson();
    await docEvent.set(json);
  }


  Stream<List<Event>> getEventByAdminId(String adminId) {
  ...
  Stream<List<Event>> getEventByCityName(String cName) {
    return eventInformation
  ...
  Future<void> removeImgFromAllComments(String img, String newImg) async {
    try {
      ...
      QuerySnapshot querySnapshot = await eventInformation.get();
  ...
  Future<int> addLikeToEvent(String eId) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();

    if (eventSnapshot.exists) {
      // If the city document exists, get the current likes count
      int currentLikes = eventSnapshot['likes'] ?? 0;

      // Increment the likes count
      int updatedLikes = currentLikes + 1;

      // Update the city document with the updated likes count
      await eventRef.update({'likes': updatedLikes});
      return updatedLikes;
    } else {
      throw Exception("Event with ID $eId does not exist");
    }
  }

  Future<int> unlikeEvent(String eId) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
    if (eventSnapshot.exists) {
      // If the city document exists, get the current likes count
  ...
  Future<String> updateEventDetail(String eid, String desc)
  async{
    try{
      await eventInformation.doc(eid).update({
        'desc': desc
      });
      return desc;
    }catch(e){
  ...
  Future<String> updateEventImg(String eid, String prevImg, File newImg)
  async{
    try{
  ...
  Stream<List<String>> imageStreamByEventId(String eId){
    return eventInformation.doc(eId).snapshots().map(_imageListFromSnapshot);
  }
  ...
  Future<void> addImage(String eId, String img) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
    if (eventSnapshot.exists) {
      List<dynamic> currentImages = eventSnapshot['images'] ?? [];
      List<String> images = currentImages.map((image) => image.toString()).toList();
      images.add(img); // Add the new image to the list
      await eventRef.update({'images': images});
    }
  }
  ...
  Future<void> deleteImageFromEvent(String eId, String imageUrl) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
  ...
  Future<void> addReview(String eId, Comment comment, Ratings ratings) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
    if (eventSnapshot.exists){
      List<dynamic> currentComment = eventSnapshot['comments'] ?? [];
      ...
      await eventRef.update({'comments': updatedComments});
      addRating(eId, ratings);
    }else {
      throw Exception("Event with ID $eId does not exist");
    }
  }

  Future<void> addRating(String eId, Ratings ratings) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
    if (eventSnapshot.exists) {
      List<dynamic> currentRatings = eventSnapshot['ratings'] ?? [];
      ...
      await eventRef.update({'ratings': updatedRatings});
    }else {
      throw Exception("Event with ID $eId does not exist");
    }
  }

  Future<void> deleteCommentByIndex(String eId, int index) async {
    ...
    if (eventSnapshot.exists) {
      ...
      List<dynamic> currentComments = eventSnapshot['comments'] ?? [];
  ...
  Future<void> addLikeToComment(String eId, int index) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
  ...
  Future<void> unlikeComment(String eId, int index) async {
    final DocumentReference eventRef = eventInformation.doc(eId);
    final DocumentSnapshot eventSnapshot = await eventRef.get();
  ...
  Stream<List<Comment>> commentsStreamByEventId(String eId) {
    return eventInformation.doc(eId).snapshots().map(_commentListFromSnapshot);
  }
  ...
  Stream<List<Ratings>> ratingsStreamByEventId(String eId) {
    return eventInformation.doc(eId).snapshots().map(_ratingsListFromSnapshot);
  }
  ...
  Future deleteEvent(String eid)async{
    final docEvent = eventInformation.doc(eid);
